...
//	private transient LSimWorker lsim = LSimFactory.getWorkerInstance();
	...
	public synchronized void updateTimestamp(Timestamp timestamp){
		...
		if( timestamp != null ) {
			// Capturamos el host del timestamp pasado por parámetro.
			String host = timestamp.getHostid();
			// Lo guardamos en el hashmap. 
			// Si la clave existe se sobreescribirá el timestamp.
	...
	public synchronized void updateMax(TimestampVector tsVector){
		...
		for(String host : this.timestampVector.keySet()) {
			...
			// Capturamos el timestamp asociado al parámetro
			Timestamp remote_ts = tsVector.getLast(host);
			// Si el local es más antiguo que el remoto, actualizamos el local.
	...
	public boolean equals(Object obj){
		// Si el objeto parámetro es nullo
		if( obj == null ) return false;
		// Si el objeto parámetro no es intancia de TimestampVector
		if ( !(obj instanceof TimestampVector) ) return false;
		// Hacemos el casting
		TimestampVector remote_ts = (TimestampVector) obj;
		// Invocamos el método equals. 
