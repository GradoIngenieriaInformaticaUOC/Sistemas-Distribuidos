...
//	private transient LSimWorker lsim = LSimFactory.getWorkerInstance();
	...
	public synchronized boolean add(Operation op){
		// Caoturamos el timestamp y el host_id de la operaci�n.
		...
		// Si hay alguna operaci�n guardada en nuestra lista.
		if(op_list.isEmpty() || op_list == null){
			top_ts = null;
		} else {
			// Capturamos el ?ltimo elemento, el m?s reciente.
			top_ts = op_list.get(op_list.size()-1).getTimestamp();
		}
		// Comparamos ambas operaciones.
		long compare_result = t.compare(top_ts);
		// Si la operaci�n a insertar es m�s reciente que la que tenemos
		if((compare_result == 1  && top_ts != null) || (compare_result == 0 && top_ts == null)) {
			// A�adimos la operaci�n a la lista
	...
	public synchronized List<Operation> listNewer(TimestampVector sum){
		// Cremos una lista de operaciones vac�a.
		...
		for( String host : this.log.keySet() ) {
			...
			// Capturamos el �ltimo timestamp del host que evaluamos del array por par�metro.
			Timestamp last_ts = sum.getLast(host);
			// Por cada operaci�n del host en nuestro log.
			for( Operation op : ops_host ) {
				// Si la oepraci�n a evaluar es 'm�s nueva' que la del par�metro la a�adimos
				// a la lista.
	...
	public boolean equals(Object obj) {
		...
		return this.log.equals(log_to_compare.log);
	}
